...
function ChartTooltip({ payload }: { payload?: Array<{ payload: Record<string, number | string> }> }) {
  ...
  return (
    <div className="rounded-lg border bg-white p-3 shadow-lg dark:border-gray-600 dark:bg-gray-800">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400">
        Data: {formatDate(d.date as number)}
      </p>
      <p className="text-sm text-gray-600 dark:text-gray-400">
        Valor: {formatCurrency(d.valor as number)}
      </p>
      <p className="text-sm text-gray-600 dark:text-gray-400">
        Preco/m²: {formatCurrency(d.preco_m2 as number)}
      </p>
      <p className="text-sm text-gray-600 dark:text-gray-400">
        Area: {d.area as number} m²
      </p>
    </div>
  );
}
    ...
    <div className="flex flex-col gap-6">
      <div className="rounded-xl bg-white p-6 shadow dark:bg-gray-900">
        <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white">
          Preco de venda ao longo do tempo
        </h3>
        <ResponsiveContainer width="100%" height={350}>
          <RechartsScatter data={chartData}>
            ...
            <Scatter dataKey="valor" fill="#10b981" />
          </RechartsScatter>
        </ResponsiveContainer>
      </div>
